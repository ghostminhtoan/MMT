...
if not defined WINPE_LANG set WINPE_LANG=VN
...
if "!WINPE_LANG!"=="VN" (
    ...
    echo.
    echo Đang quét ổ cứng...
    diskpart /s nul 2>nul
    for /f "skip=1 tokens=1,2" %%a in ('wmic logicaldisk get caption^,size 2^>nul') do (
        if not "%%a"=="" echo   %%a
    )
    echo.
    echo z - Quay lại menu chính
...
if "!format!"=="y" (
    if "!WINPE_LANG!"=="VN" (
        ...
        pause
        echo format !drive! /fs:ntfs /q /y
        echo Format đã hoàn thành [Mô phỏng]
    ) else (
        echo Formatting drive !drive!...
        echo [WARNING] All data on drive !drive! will be deleted!
        pause
        echo format !drive! /fs:ntfs /q /y
        echo Format completed [Simulation]
    )
)
